...
export default function SupportPage({
  ...
  return (
    <div className="space-y-6" id="support_root">
      {/* Page Header */}
      <div className="p-6 bg-[#0f1424] rounded-2xl border border-slate-900" id="support_header">
        ...
        <div className="md:col-span-6 space-y-6" id="support_queries_box">
          <div className="p-5 bg-slate-900 rounded-xl border border-slate-800 space-y-4">
            ...
            <form onSubmit={handleTicketSubmit} className="space-y-3" id="ticket_form">
              {ticketSuccess && (
                <div className="p-3 bg-emerald-950/20 border border-emerald-500/20 rounded-lg text-emerald-400 text-xs flex gap-2">
              ...
              <div className="space-y-1">
                ...
                <textarea
                  rows={4}
                  className="w-full bg-[#151c2e] border border-slate-850 rounded-lg p-3 text-xs text-white focus:outline-none focus:border-indigo-505"
            ...
            {userQueries.length === 0 ? (
              ...
              <div className="space-y-3 max-h-56 overflow-y-auto pr-1" id="tickets_pile">
                {userQueries.map((q) => (
                  <div 
                    key={q.id} 
                    className="p-3 bg-slate-950/40 rounded-xl border border-slate-850 text-xs space-y-2 text-left"
                  ...
                  >
                    <div className="flex justify-between items-center text-[10px] text-gray-500 font-semibold uppercase">
                      ...
                      <span className={`px-2 py-0.2 rounded font-extrabold ${q.status === 'Resolved' ? 'bg-emerald-950/30 text-emerald-400 border border-emerald-500/20' : 'bg-amber-950/30 text-amber-400 border border-amber-500/20'}`}>
                        {q.status}
                      </span>
                    </div>
        ...
        <div className="md:col-span-6" id="support_oac_box">
          <div className="p-5 bg-slate-900 rounded-xl border border-slate-800 space-y-4">
            ...
            <form onSubmit={handleOacSubmit} className="space-y-3" id="oac_form">
              ...
              {oacError && (
                <div className="p-3 bg-red-955/20 border border-red-500/20 rounded-lg text-red-400 text-xs flex gap-2">
              ...
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  disabled
                  className="w-full bg-[#1d263a] border border-slate-850 rounded-lg py-1.5 px-3 text-xs text-gray-400 cursor-not-allowed font-medium"
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    type="url"
                    className="w-full bg-[#151c2e] border border-slate-850 rounded-lg py-1.5 pl-14 pr-3 text-xs text-white focus:outline-none focus:border-amber-500"
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    type="url"
                    className="w-full bg-[#151c2e] border border-slate-850 rounded-lg py-1.5 pl-16 pr-3 text-xs text-white focus:outline-none focus:border-amber-500"
                ...
                {userOacs.map((app) => (
                  ...
                  >
                    ...
                    <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-black ${
                      app.status === 'Approved' ? 'bg-emerald-950/30 text-emerald-450 border border-emerald-500/20 text-emerald-400' :
