    ...
    <div className="space-y-6 animate-fade-in" id="revenue_root_module">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6" id="revenue_stats_mesh">
        ...
        <div 
          ...
          className="p-5 rounded-2xl bg-[#12163b]/30 border border-emerald-500/10 hover:border-emerald-500/30 flex items-center justify-between cursor-pointer transition relative"
        ...
        >
          <div className="space-y-1 min-w-0 flex-1 pr-2">
            <span className="text-[10px] text-emerald-400 font-extrabold uppercase tracking-widest block flex items-center gap-1.5">
              Cumulative Earnings <span className="text-[8px] bg-emerald-500/15 text-emerald-300 px-1 py-0.5 rounded border border-emerald-500/20 font-sans tracking-normal uppercase">Statements</span>
            </span>
            <div className="space-y-0.5 font-mono">
              ...
              {totalUSD > 0 && (
                <div className="text-sm font-semibold text-emerald-400">{formatAmount(totalUSD, 'USD')}</div>
              )}
          ...
          <div className="p-3 bg-emerald-500/10 text-emerald-400 rounded-full flex-shrink-0">
            <DollarSign className="w-5 h-5" />
          </div>
        </div>

        {/* Metric Card 2: Available to Withdraw */}
        <div className="p-5 rounded-2xl bg-gradient-to-r from-emerald-950/20 to-teal-950/20 border border-emerald-500/20 flex items-center justify-between">
          <div className="space-y-1 min-w-0 flex-1 pr-2">
            <span className="text-[10px] text-emerald-300 font-extrabold uppercase tracking-widest block">Withdrawable Balance</span>
            <div className="space-y-0.5 font-mono">
              {availableINR > 0 && (
                <div className="text-xl sm:text-2xl font-black text-white">{formatAmount(availableINR, 'INR')}</div>
              )}
              {availableUSD > 0 && (
                <div className="text-sm font-semibold text-emerald-400">{formatAmount(availableUSD, 'USD')}</div>
              )}
          ...
          <div className="p-3 bg-emerald-500/10 text-[#1DB954] rounded-full flex-shrink-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6" id="revenue_interaction_mesh">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-900/60 p-5 rounded-2xl border border-slate-800 space-y-4" id="payout_destination_config">
              <div>
                <h3 className="text-xs font-black text-gray-200 uppercase tracking-wider flex items-center gap-1.5">
                  <CreditCard className="w-4 h-4 text-emerald-400" /> Payout Destination
                </h3>
              ...
              <div className="flex gap-2 p-1 bg-black/40 rounded-lg">
                <button
                  ...
                  className={`flex-1 py-1.5 rounded text-[10px] font-bold uppercase transition ${
                    activeConfigTab === 'upi' ? 'bg-[#1DB954] text-black' : 'text-slate-400 hover:text-white'
                  }`}
                ...
                <button
                  type="button"
                  className={`flex-1 py-1.5 rounded text-[10px] font-bold uppercase transition ${
                    activeConfigTab === 'bank' ? 'bg-[#1DB954] text-black' : 'text-slate-400 hover:text-white'
                  }`}
              ...
              <form onSubmit={handleSavePaymentConfig} className="space-y-3">
                {activeConfigTab === 'upi' ? (
                  <div className="space-y-1.5 text-left">
                    ...
                    <input
                      type="text"
                      className="w-full text-xs bg-slate-950 border border-slate-800 p-2.5 rounded-lg text-white font-mono placeholder:text-slate-600 focus:outline-none focus:border-emerald-500"
                  ...
                  <div className="space-y-3">
                    <div className="space-y-1 text-left">
                      ...
                      <input
                        type="text"
                        className="w-full text-xs bg-slate-950 border border-slate-800 p-2 rounded text-white font-medium focus:outline-none focus:border-emerald-500"
                    ...
                    <div className="grid grid-cols-2 gap-3 text-left">
                      <div className="space-y-1">
                        ...
                        <input
                          type="text"
                          className="w-full text-xs bg-slate-950 border border-slate-800 p-2 rounded text-white font-mono focus:outline-none focus:border-emerald-500"
                ...
                {saveSuccessMsg && (
                  <p className="text-[10px] font-bold text-emerald-400 mt-2">{saveSuccessMsg}</p>
                )}

                <button
                  type="submit"
                  className="w-full py-2 bg-slate-800 hover:bg-slate-700 font-bold text-xs text-white uppercase rounded-xl transition cursor-pointer mt-2"
                >
            ...
            <div className="bg-slate-900/60 p-5 rounded-2xl border border-slate-800 space-y-4" id="request_withdrawal_payout">
              <div>
                <h3 className="text-xs font-black text-gray-200 uppercase tracking-wider flex items-center gap-1.5">
                  <Send className="w-4 h-4 text-[#1DB954]" /> Request Payout
                </h3>
                <p className="text-[10px] text-slate-400 mt-1">
              ...
              <form onSubmit={handleWithdrawPrompt} className="space-y-3">
                <div className="grid grid-cols-3 gap-2">
                  <div className="col-span-2 space-y-1 text-left">
                    ...
                    <input
                      ...
                      step="any"
                      min="0.01"
                      className="w-full text-xs bg-slate-950 border border-slate-800 p-2 rounded-lg text-white font-mono placeholder:text-slate-700 focus:outline-none focus:border-emerald-500 text-left"
                      placeholder="e.g. 150"
                  ...
                  <div className="space-y-1 text-left">
                    ...
                    <select
                      className="w-full text-xs bg-slate-950 border border-slate-800 p-2 rounded-lg text-white font-bold h-[34px] focus:outline-none focus:border-emerald-500"
                ...
                <div className="space-y-1 text-left">
                  ...
                  <select
                    className="w-full text-xs bg-slate-950 border border-slate-800 p-2 rounded-lg text-white font-medium focus:outline-none focus:border-emerald-500"
                ...
                {withdrawSuccess && (
                  <p className="text-[10px] font-bold text-emerald-400" id="withdraw_success_log">
                ...
                <button
                  ...
                  className="w-full py-2 bg-[#1DB954] hover:bg-[#1ed760] disabled:bg-zinc-800 font-extrabold text-xs text-black uppercase rounded-xl transition cursor-pointer mt-1"
                >
          ...
          <div className="bg-[#0f1424]/40 p-5 rounded-2xl border border-slate-900 space-y-4" id="withdrawal_requests_table_box">
                          ...
                          <div className="space-y-0.5">
                            {req.status === 'Pending' ? (
                              ...
                              <span className="text-emerald-400 font-black flex items-center gap-1 text-[10px]">
                                <CheckCircle className="w-3.5 h-3.5 text-emerald-400" /> Dispatched
                              </span>
                            ) : (
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="p-5 bg-slate-900 rounded-2xl border border-slate-800 space-y-4" id="revenue_distribution_overview">
            <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
            ...
            {userReports.length === 0 ? (
              ...
              <div className="space-y-3">
                {userReports.map((item) => (
                  <div key={item.id} className="p-3 bg-black/40 rounded-xl border border-slate-950 flex items-center justify-between text-xs">
                    <div>
                    ...
                    <div className="text-right">
                      <span className="font-bold text-emerald-400 block">{formatAmount(item.amount, item.currency)}</span>
