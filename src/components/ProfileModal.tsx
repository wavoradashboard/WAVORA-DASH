...
export default function ProfileModal({ isOpen, onClose, currentUser, onSavePassword }: ProfileModalProps) {
  ...
  const getPlanBadge = (plan: 'Basic' | 'Pro' | 'Elite') => {
    switch (plan) {
      case 'Elite':
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-black tracking-widest uppercase bg-gradient-to-r from-amber-500/25 to-yellow-500/10 border border-amber-500 text-amber-400 shadow-[0_0_12px_rgba(245,158,11,0.25)]" id="badge_elite">
            ★ Elite Artist
          </span>
        );
      case 'Pro':
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-black tracking-widest uppercase bg-gradient-to-r from-[#1DB954]/25 to-emerald-500/10 border border-[#1DB954] text-[#1DB954] shadow-[0_0_12px_rgba(29,185,84,0.25)]" id="badge_pro">
            ✦ Pro Distribution
          </span>
        );
      default:
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" id="profile_modal_root">
      {/* Backdrop */}
      <div 
      ...
      >
        {/* Header bar */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#1F1F1F] bg-[#0A0A0A]" id="profile_modal_header">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-full bg-[#1DB954]/10 border border-[#1DB954]/25 flex items-center justify-center">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6" id="profile_modal_body">
          ...
          <div className="p-5 bg-[#141414] rounded-xl border border-[#1E1E1E] space-y-4" id="profile_user_details">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-[#1DB954] text-black font-black text-2xl flex items-center justify-center shadow-lg">
          ...
          <div className="p-5 bg-[#141414] rounded-xl border border-[#1E1E1E] text-left" id="profile_change_password">
            <span className="text-xs font-black text-white uppercase tracking-widest block mb-4 flex items-center gap-1.5 border-b border-[#1F1F1F] pb-2">
              <KeyRound className="w-3.5 h-3.5 text-[#1DB954]" /> Change Password Section
            </span>
            ...
            <form onSubmit={handlePasswordChangeSubmit} className="space-y-4">
              ...
              {successMsg && (
                <div className="p-2.5 text-xs text-emerald-400 bg-emerald-950/20 border border-emerald-500/20 rounded font-mono flex items-center gap-1.5" id="profile_password_success">
              ...
              <div className="flex justify-end">
                <button
                  type="submit"
                  className="px-5 py-2.5 bg-[#1DB954] text-black hover:bg-[#1ed760] font-black text-xs uppercase tracking-wider rounded transition cursor-pointer"
          ...
          <div className="p-5 bg-[#141414] rounded-xl border border-[#1E1E1E] text-left" id="profile_terms_section">
            <span className="text-xs font-black text-white uppercase tracking-widest block mb-3 flex items-center gap-1.5 border-b border-[#1F1F1F] pb-2">
              <FileText className="w-3.5 h-3.5 text-gray-400" /> Platform Rules & Distribution Agreement
            </span>

            <div 
              className="h-32 rounded bg-black border border-[#1F1F1F] p-3 text-[11px] text-gray-400 font-sans overflow-y-auto leading-relaxed space-y-2.5 scrollbar-thin shadow-inner"
