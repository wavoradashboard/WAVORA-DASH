...
export default function ProfileModal({ isOpen, onClose, currentUser, onSavePassword }: ProfileModalProps) {
  ...
  const getPlanBadge = (plan: 'Basic' | 'Pro' | 'Elite') => {
    switch (plan) {
      ...
      case 'Pro':
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-black tracking-widest uppercase bg-gradient-to-r from-[#6366F1]/25 to-indigo-500/10 border border-[#6366F1] text-[#6366F1] shadow-[0_0_12px_rgba(29,185,84,0.25)]" id="badge_pro">
            ✦ Pro Distribution
          </span>
        );
      default:
